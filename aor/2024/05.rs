...
impl Kid {
    pub fn parse_row(csv_row: &str) -> Result<Kid, &'static str> {
        ...
        // 🎁 Your code here! 🎁
        let mut token = csv_row.split(",");
        let name = token.next().ok_or("Missing field")?.to_string();
        let good_deeds = token.next().ok_or("Missing field")?.parse::<u32>().map_err(|_| "NaN")?;
        let bad_deeds = token.next().ok_or("Missing field")?.parse::<u32>().map_err(|_| "NaN")?;
